        ...
        try {
            ...
            if (data.users && data.users.length > 0) {
                resultsDiv.innerHTML = data.users
                    ...
                    .map(u => `
                        <div style="padding: 8px; background: var(--macos-bg-secondary); margin-bottom: 5px; border-radius: var(--macos-radius-small); cursor: pointer; display: flex; justify-content: space-between; align-items: center;"
                             onclick="selectUser(${u.id}, '${u.name}', '${u.email}')">
                            <div>
                                <strong>${u.name || 'Unnamed'}</strong>
                                <div style="font-size: 12px; color: var(--macos-text-secondary);">${u.email}</div>
                            </div>
                            <button onclick="event.stopPropagation(); selectUser(${u.id}, '${u.name}', '${u.email}')"
                                    style="background: var(--macos-blue); color: white; border: none; padding: 4px 12px; border-radius: var(--macos-radius-small); cursor: pointer;">
                                Selecteer
                            </button>
                        </div>
                    `).join('');
...
function updateSelectedUsersDisplay() {
    ...
    if (selectedUserIds.length === 0) {
        selectedDiv.innerHTML = '<div style="color: var(--macos-text-secondary); font-size: 14px;">Geen gebruikers geselecteerd</div>';
        return;
    }

    selectedDiv.innerHTML = `
        <div style="margin-bottom: 8px; font-weight: 600; color: var(--macos-text-primary);">
            Geselecteerde gebruikers (${selectedUserIds.length}):
        </div>
        ${selectedUserIds.map(id => `
            <div style="display: inline-flex; align-items: center; background: var(--macos-blue); color: white; padding: 6px 12px; border-radius: var(--macos-radius-small); margin: 4px;">
                <span style="margin-right: 8px;">User ID: ${id}</span>
                <button onclick="removeUser(${id})"
                        style="background: rgba(255,255,255,0.2); border: none; color: white; padding: 2px 8px; border-radius: var(--macos-radius-small); cursor: pointer; font-weight: bold;">
                    ×
                </button>
            </div>
        `).join('')}
        ...
        try {
            ...
            contentDiv.innerHTML = `
                <div style="font-size: 16px; font-weight: 600; color: var(--macos-blue); margin-bottom: 8px;">
                    ${data.count} gebruiker${data.count !== 1 ? 's' : ''} zullen dit bericht ontvangen
                </div>
                ${data.sample && data.sample.length > 0 ? `
                    <div style="font-size: 14px; color: var(--macos-text-secondary); margin-top: 8px;">
                        Voorbeelden: ${data.sample.map(u => u.name || u.email).join(', ')}
                    </div>
                ` : ''}
            `;
        } catch (error) {
...
async function handleMessageSubmit(e) {
    ...
    const statusDiv = document.getElementById('messageFormStatus');
    ...
    try {
        ...
        statusDiv.textContent = '⏳ Bericht aanmaken...';
        ...
        const messageData = {
            ...
            message_type: 'information', // Phase 3 will add type selector
            target_type: targetType,
            target_subscription: targetSubscription,
            target_users: targetUsers,
            trigger_type: triggerType,
            trigger_value: triggerValue,
            dismissible: true, // Phase 3 will add checkbox
            snoozable: true, // Phase 3 will add checkbox
            publish_at: publishAt ? new Date(publishAt).toISOString() : null,
            expires_at: expiresAt ? new Date(expiresAt).toISOString() : null,
            button_label: null, // Phase 3 will add button config
            button_action: null,
            button_target: null
        };
        ...
        if (response.ok) {
            statusDiv.style.background = 'var(--toast-success-bg)';
            statusDiv.style.color = 'var(--toast-success)';
            statusDiv.textContent = `✅ Bericht succesvol aangemaakt! (ID: ${result.messageId})`;
        ...
        statusDiv.style.background = 'var(--toast-error-bg)';
        statusDiv.style.color = 'var(--toast-error)';
...
function resetMessageForm() {
    ...
    selectedUserIds = [];
    updateSelectedUsersDisplay();
    document.getElementById('messageFormStatus').style.display = 'none';
    document.getElementById('targetPreview').style.display = 'none';
